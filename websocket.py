...
import sys; sys.path.insert(0, "/opt/honeyradar")
...
MALWARE_LOGS = [
    ...
    for p in os.getenv(
        "MALWARE_LOGS",
        "/var/tmp/honeyradar-malware.log",
    ).split(",")
    if p.strip()
]
...
STORE = HoneyRadarStore(os.getenv("HONEYRADAR_DB", "/opt/honeyradar/honeyradar.db"))
...
async def process_line(source: str, line: str) -> None:
    ...
    try:
        ...
        if source == "malware_scanner":
            event["source"] = "malware_scanner"
            event.setdefault("eventid", "honeyradar.malware.sample")
            STORE.record(event)
            await broadcast(json.dumps(event, separators=(",", ":")))
            return
...
async def main() -> None:
    ...
    tasks = [
        ...
        asyncio.create_task(tail_first_available(MALWARE_LOGS, "malware_scanner")),
    ]
